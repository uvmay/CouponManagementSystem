...
function respondError(reason){
	   ...
	   if(reason.status == 401)
		   window.location.href="http://localhost:8080/Coupon_System_Project/UnauthorizedError.html?error=Unauthorized! Login With Proper User";
	   else
 ...
 * and reboots the shopping cart*/
	$scope.logoutFunc = function(){
		console.log("clicked");
	$http.get("http://localhost:8080/Coupon_System_Project/rest/login")
...
$scope.general.retrieveCart = function(){
	$http.get("http://localhost:8080/Coupon_System_Project/rest/coupon/cart/get/customer")
	.then(function(response){
...
$scope.general.updateCart = function(){
	   var JSONcart = JSON.stringify($scope.general.shoppingCart); 
	    $http.post("http://localhost:8080/Coupon_System_Project/rest/coupon/cart/save/customer",JSONcart);   
};
