 ...
 * searches by the search selector, and validates the search data when needed*/
	$scope.search.funct=function(){
		...
		//a request for coupons which name contains the string in the search bar is sent to the server
		   ...
		   $http.get("http://localhost:8081/rest/coupon/purchased/get/by/string/"+$scope.search.value+"/customer")
		...
		//a request for coupons with the selected type is sent to the server
		   ...
		   $http.get("http://localhost:8081/rest/coupon/purschased/get/by/type/"+$scope.search.type.selector.toUpperCase()+"/customer")
		   .then(function(response){
		   $("#loader").hide();
		   $scope.coupons = response.data;
				},function(reason){
		...
		//a request for coupons with the selected price is sent to the server
		   ...
		   if(!isNaN(parseInt($scope.search.value))){
				$http.get("http://localhost:8081/rest/coupon/purschased/get/by/price/"+parseFloat($scope.search.value)+"/customer")
				.then(function(response){
					$("#loader").hide();
					$scope.coupons = response.data;
					 },function(reason){
...
/**a function that gets all of the coupons from the server */
	$scope.getAllCoupons = function(){
		$("#loader").show();
		$http.get("http://localhost:8081/rest/coupon/purchased/get/all/customer")
		.then(function(response){
		    $("#loader").hide();
		    $scope.coupons = response.data;
		},respondError);
