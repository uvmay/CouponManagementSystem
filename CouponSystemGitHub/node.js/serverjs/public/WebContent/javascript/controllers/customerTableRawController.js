 ...
 * @param fields.password = the password field when editing */
	...
	$scope.fields={};
	$scope.fields.name=$scope.line.username;
...
/**the function that sends the edited company to the server */
		 this.doneEditing = function(){
			...
			newCust.username=$scope.fields.name;
			newCust.password=$scope.fields.password;
			
			var JSONNewCust = JSON.stringify(newCust);
			$("#loader").show();
			$http.put("http://localhost:8081/rest/customer/update",JSONNewCust)
	    	.then(
	    	 function(response){
 ...
 * @param compId = the deleted company's id */
	$scope.deleteCustomer = function(custId){
		$("#loader").show();
		$http.delete("http://localhost:8081/rest/customer/remove/"+custId)
		.then(function(){
			$scope.getAllCustomers();
			},function(reason){
				  $("#loader").hide();
