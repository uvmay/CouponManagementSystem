 ...
 * containing the username, password and user type */
		...
		var timer = $scope.loadLogin();
	    	$http.post("http://localhost:8081/rest/login",JSONLoginObject)
	    	.then(function(response){
	    		...
	    		if($scope.selectType=="Admin")
	    		   window.location.href="http://localhost:8081/#/admin/companies";
	    		else if($scope.selectType=="Company")
		    	   window.location.href="http://localhost:8081/#/company";
	    		else
			       window.location.href="http://localhost:8081/#/customer/store";
