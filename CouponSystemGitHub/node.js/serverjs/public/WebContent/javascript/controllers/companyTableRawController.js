...
/**the function that sends the edited company to the server */
		 this.doneEditing = function(){
			...
			newComp.username=$scope.fields.name;
			...
			$http.put("http://localhost:8081/rest/company/update",JSONNewComp)
	    	.then(
	    		function(response){
 ...
 * @param compId = the deleted company's id */
	$scope.deleteCompany = function(compId){
		$("#loader").show();
		$http.delete("http://localhost:8081/rest/company/remove/"+compId)
		.then(function(){
			$scope.getAllCompanies();
			},respondError);
	};
