 ...
 * @param newImageUrl = the URL of the latest uploaded image */
    ...
    $scope.newCoupon.image="WebContent/images/No_image_available.jpg";
	$scope.editCoupon={};
	$scope.hides={};
...
/**a function that gets all of the company's coupons in the system from the server */
	$scope.getAllCoupons = function(){
		$http.get("http://localhost:8081/rest/coupon/get/all/company")
 ...
 * it sends the object "newCoupon"*/
	$scope.createCoupon = function(){
		...
		$("#loader").show();
		$http.post("http://localhost:8081/rest/coupon/create/company",JSONNewCoupon)
    	.then(
    		function(response){
	...
	$scope.deleteAllCheckedCompanyCoupons = function(){
		...
		for(var i=0;i<$scope.boxes.checkedBoxes.length;i++){
			if($scope.boxes.checkedBoxes[i]!=null && $scope.boxes.checkedBoxes[i]!=undefined )
			   $http.delete("http://localhost:8081/rest/coupon/remove/"+$scope.boxes.checkedBoxes[i]+"/company")
			   .then(function(){
	...
	$scope.search.funct=function(){
		...
		$scope.search.date.showHide==true))
		  ...
		  if($scope.search.selector=="Id"){
			...
			$http.get("http://localhost:8081/rest/coupon/get/id/"+parseInt($scope.search.value)+"/company")
			.then(function(response){
				$("#loader").hide();
				$scope.coupons = [response.data];
			  },function(reason){
		  ...
		  //a request for coupons which name contains the string in the search bar is sent to the server
			...
			$http.get("http://localhost:8081/rest/coupon/get/by/string/"+$scope.search.value+"/company")
			.then(function(response){
				$("#loader").hide();
				$scope.coupons = response.data;
			  },function(reason){
		  ...
		  //a request for coupons with the selected type is sent to the server
				...
				$http.get("http://localhost:8081/rest/coupon/get/by/type/"+$scope.search.type.selector.toUpperCase()+"/company")
				.then(function(response){
					$("#loader").hide();
					$scope.coupons = response.data;
				  },function(reason){
		  ...
		  //a request for coupons with the selected expiration date is sent to the server
			   ...
			   console.log($scope.search.date.value);
				$http.get("http://localhost:8081/rest/coupon/get/by/date/"+Date.parse($scope.search.date.value)+"/company")
				.then(function(response){
					$("#loader").hide();
					$scope.coupons = response.data;
				  },function(reason){
		  ...
		  //a request for coupons with the selected price is sent to the server
			  ...
			  if(!isNaN(parseInt($scope.search.value))){
				$http.get("http://localhost:8081/rest/coupon/get/by/price/"+parseFloat($scope.search.value)+"/company")
				.then(function(response){
					$("#loader").hide();
					$scope.coupons = response.data;
				  },function(reason){
...
/**a function that sends a coupon to the server after it was edited */
	$scope.doneEditingCoupon = function(){
		   ...
		   $("#loader").show();
		   $http.put("http://localhost:8081/rest/coupon/update/company",JSONEditCoup)
    	   .then(
    		   function(response){
    			   //if the response is valid, updates the coupons table and hides
...
/**a function that happens on a coupon image selection - it uploads the image to the server*/
    $scope.onFileSelect = function($file) {
          var file=$file;
          Upload.upload({
            url: "http://localhost:8081/rest/upload",
            data: {file:file},
            progress: function(e){}
          }).then(function(response) {
